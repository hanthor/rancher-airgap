...
**Example**: `hauler/rke2/rancher-airgap-rke2.yaml` contains RKE2 installation files, container images, and SELinux RPMs.
...
# Fetch and transform image lists from upstream
curl -sSfL https://github.com/rancher/rke2/.../rke2-images-all.linux-amd64.txt | 
  sed -e "s/docker\.io\///g" -e "s/^/    - name: /"

# Generate YAML manifest with heredoc
cat << EOF >> rancher-airgap-rke2.yaml
apiVersion: content.hauler.cattle.io/v1
kind: Images
...
EOF
...
Downstream RKE2/K3s installations configure `system-default-registry` to point to the Hauler registry, while installation scripts fetch from the fileserver.

## Project Conventions

### Version Management

- Product versions are hardcoded in manifests (e.g., `RKE2: v1.33.5`)
- Update both the manifest YAML AND the corresponding generator script
- Version variables use consistent naming: `vRKE2`, `vRancher`, `vCertManager`

### Minimal vs Full Manifests

Some products provide two variants:

- `rancher-airgap-rancher.yaml` - Full deployment with all images
- `rancher-airgap-rancher-minimal.yaml` - Core components only

**When to use**: Minimal manifests reduce transfer size for bandwidth-constrained airgaps.

### Platform Architecture

Default platform is `linux/amd64`, but manifests include `linux/arm64` artifacts for:

- RKE2/K3s installation tarballs
- Container images (fetched from multi-arch lists)
...
Use the quickstart workflow from `examples/rancher-airgap-quickstart.md`:
...
- **Hauler**: The core tool (https://github.com/hauler-dev/hauler) - manifests define WHAT to collect, Hauler performs the collection/packaging
- **Upstream sources**: GitHub releases, Helm repositories, official image lists
- **OS packages**: Uses `repotrack` for RPM dependencies (see `examples/os-packages-example.md`)

## Common Pitfalls

1. **Registry configuration**: RKE2/K3s `registries.yaml` requires both `mirrors` and `configs` sections, even for HTTP registries
2. **Store naming**: Use consistent naming pattern `<product>-store` to avoid cross-product contamination
3. **File URLs**: Percent-encode special characters in GitHub release URLs (e.g., `+` becomes `%2B`)
4. **Image references**: Remove `docker.io/` prefix in image lists since Hauler normalizes registry paths
...
- **Update RKE2 version**: `hauler/scripts/rke2/hauler-rke2.sh`, then regenerate manifest
- **Add new Helm chart**: Update `Charts` section in relevant manifest YAML
- **Modify quickstart**: `examples/rancher-airgap-quickstart.md` (referenced by users)
- **View supported versions**: Root `README.md` "Repository Structure" section
